...
class KibanaClient:
    def __init__(self, base_url: str = None, es_host: str = None, index_pattern: str = "logs-*"):
    ...
    def search(self, query: dict = None, start: datetime = None, end: datetime = None, size: int = 100) -> list:
        ...
        if start and end:
            must.append({"range": {"@timestamp": {"gte": start.isoformat(), "lte": end.isoformat()}}})
    ...
    def search_errors(self, start: datetime = None, end: datetime = None, level: str = "ERROR") -> list:
        query = {"term": {"level": level}}
        return self.search(query, start, end)

    def get_error_count(self, start: datetime = None, end: datetime = None) -> int:
    ...
    def get_logs_by_service(self, service: str, start: datetime = None, end: datetime = None) -> list:
    ...
    def get_service_errors(self, service: str, minutes: int = 30) -> list:
        start = datetime.utcnow() - timedelta(minutes=minutes)
        end = datetime.utcnow()
        query = {"bool": {"must": [{"term": {"service.name": service}}, {"term": {"level": "ERROR"}}]}}
        return self.search(query, start, end)
